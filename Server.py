...
def handle_get_contacts(connectionSocket: socket, username: str | None, db_local: DB):
    ...
    Contacts are derived from message history: anyone the user has chatteed a
...
def handle_search(connectionSocket: socket, username: str | None, temp: list, db_local: DB):
    '''Search for usernames by substring, meaning if part of that string matches any user name, that username is returned.'''
    if not username:
        send_message(connectionSocket, "ERROR|NOT_LOGGED_IN\n\n")
        return
...
def handle_account_creation(connectionSocket: socket, temp: list, db_local: DB):
    """Handle `CREATE` requests to register a new user. We recive a username and password from the client, check if the credentials are valid and wether the user already exists. 
    If the user does not exist, we create the user and send a success message. If the user already exists, we send an error message.
    """
    if len(temp) < 3:
...
def handle_private_message(connectionSocket: socket, sender_username: str | None, temp: list, db_local: DB):
    """Store a private message and optionally push it in realtime, depenfing on wether the person you're cahtting with is online and has your chat open.
...
def handle_group_open(connectionSocket: socket, username: str | None, temp: list, db_local: DB):
    ...
    It might be weird, but the user can see messgaes from before they joined the group. We're taking a similar approach to stuff like tellegram groupchats.
    """
    if not username:
        send_message(connectionSocket, "ERROR|NOT_LOGGED_IN\n\n")
        return
...
def send_message(connectionSocket: socket, message: str) -> None:
    '''Send a TCP message to a client.'''
